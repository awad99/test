#pragma once
#ifndef MY_OBFUSCTION_H
#define MY_OBFUSCTION_H
...
inline std::string encode_string(const std::string& str) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        encoded += static_cast<char>(str[i] ^ (myXorConst >> (i % 64)) + myOffsetConst);
    }
    return encoded;
}
...
inline std::string decode_string(const std::string& encoded) {
    ...
    for (size_t;  i = 0; i < encoded.size(); ++i) {
        decoded += static_cast<char>(encoded[i] ^ (myXorConst >> (i % 64)) - myOffsetConst);
    }
    return decoded;
}

// Macro for obfuscating strings during compile-time
#define OBF_STRING(str) encode_string(str)

// Macro to demonstrate how to use the obfuscated string
//#define PRINT_OBF_STRING(str) std::cout << "Obfuscated: " << encode_string(str) << std::endl; \
                               std::cout << "Decoded: " << decode_string(encode_string(str)) << std::endl;
